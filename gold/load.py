...
def load_dim_race(engine):
    ...
    dim_race=pd.read_sql("select distinct \"raceId\", \"year\", \"round\", \"date\", \"name_x\", \"url_x\", \"quali_date\", \"quali_time\", \"time_races\", \"sprint_date\", \"sprint_time\" from silver_row", engine)
...
def load_dim_driver(engine):
    Base.metadata.create_all(engine)
    dim_driver=pd.read_sql("select distinct \"driverId\", \"driverRef\", \"number\", \"code\", \"forename\", \"surname\", \"nationality\", \"url\" from silver_row", engine)
    with engine.connect() as conn:
...
def load_dim_constructors(engine):  
    Base.metadata.create_all(engine)
    dim_constructors=pd.read_sql("select distinct \"constructorId\", \"constructorRef\", \"name\", \"nationality_constructors\", \"url_constructors\" from silver_row", engine)
    with engine.connect() as conn:
...
def load_dim_circuit(engine):
    Base.metadata.create_all(engine)
    dim_circuit=pd.read_sql("select distinct \"circuitId\", \"circuitRef\", \"name_y\", \"location\", \"country\", \"lat\", \"lng\", \"alt\", \"url_y\" from silver_row", engine)
    with engine.connect() as conn:
...
def load_dim_constructorstandings(engine):
    Base.metadata.create_all(engine)
    dim_constructorstandings=pd.read_sql("select distinct \"constructorStandingsId\", \"points_constructorstandings\", \"position_constructorstandings\", \"wins_constructorstandings\", \"positionText_constructorstandings\" from silver_row", engine)
    with engine.connect() as conn:
...
def load_fact(engine):
    Base.metadata.create_all(engine)
    fact=pd.read_sql("select \"id\", \"resultId\", \"raceId\", \"driverId\", \"constructorId\", \"statusId\",\"circuitId\",\"driverStandingsId\",\"constructorStandingsId\", \"dateId\", \"points\", \"position\", \"positionText\", \"positionOrder\", \"grid\", \"laps\", \"time\", \"milliseconds\", \"rank\", \"fastestLap\",\"fastestLapTime\", \"fastestLapSpeed\" from silver_row", engine)
    with engine.connect() as conn:
...
def load_fact_lap(engine):
    Base.metadata.create_all(engine)
    fact_lap=pd.read_sql("select  \"raceId\", \"driverId\", \"lap\", \"position_laptimes\", \"milliseconds_laptimes\", \"time_laptimes\" from silver_row", engine)
    with engine.connect() as conn:
...
def load_fact_lappitstops(engine):
    Base.metadata.create_all(engine)
    fact_lappitstops=pd.read_sql("select  \"raceId\", \"driverId\", \"stop\", \"milliseconds_pitstops\", \"time_pitstops\", \"duration\" from silver_row", engine)
    with engine.connect() as conn:
